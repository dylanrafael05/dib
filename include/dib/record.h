...
namespace dib
{
    ...
    struct Record : HashAsRecord, CompareAsRecord {};
...
/// this behaviour.
template<dib::annotated_with<dib::HashAsRecord> T>
struct std::hash<T> 
{
...
template<dib::annotated_with<dib::HashByKey> T>
struct std::hash<T> 
{
    constexpr size_t operator()(const T &value) const
    {
...
template<dib::annotated_with<dib::ProvidesHash> Hashable>
...
/// operator==.
template<dib::annotated_with<dib::CompareAsRecord> T>
...
template<dib::annotated_with<dib::CompareByKey> T>
constexpr bool operator==(const T &lhs, const T &rhs)
{
    template for(constexpr auto field : std::define_static_array(
        std::meta::nonstatic_data_members_of(^^T, std::meta::access_context::unchecked())))
